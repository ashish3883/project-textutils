...
export default function ToDos(props) {
    ...
    return (
    <>
       <h1 style={{display:'flex', justifyContent:'center', color:props.mode === 'light'?'black':'white'}}>TO-DO LIST</h1>
       <div className="input-group mb-3 ">
            <input type="text" 
            className="form-control col-2"
            style={{color:props.mode==='light'?'black':'white', backgroundColor:props.mode==='light'?'white':'#333333' }} 
            value={inputVal} 
            ...
            />
            <button 
            className={`input-group-text btn btn-${props.mode==='dark'?'secondary':'primary'} mx-1`} id="basic-addon2"
            onClick={() => {
                dispatcher({
                    type: 'ADD_ITEM',
                    title: inputVal,
            ...
            >Add Item</button>
        <select 
            className={`btn btn-${props.mode==='dark'?'secondary':'primary'} mx-1`} 
